...
class Pos extends MY_Controller {
	...
	public function index()
	{
		...
		//$to_day = date("d-m-Y",strtotime($today));
  		$license = '4lUre9E5LM93zg==';
		$ciphering = "AES-128-CTR";
		$decryption_key = "GeeksforGeeks";
		$options = 0;
		$decryption_iv = '1234567891011121';
		$decryption=openssl_decrypt ($license, $ciphering, $decryption_key, $options, $decryption_iv);
	...
	//Print guest POS invoice 
	public function print_guest_invoice_pos($hold_id){
		if(!$this->permissions('sales_add') && !$this->permissions('sales_edit')){
			$this->show_access_denied_page();
		}
		$data=$this->data;
		$data['page_title']=$this->lang->line('sales_invoice');
		$data=array_merge($data,array('hold_id'=>$hold_id));
		$this->load->view('sal-guest-invoice-pos',$data);
	}

	//Print Kitchen POS invoice 
	public function print_kitchen_invoice_pos($sales_id){
		if(!$this->permissions('sales_add') && !$this->permissions('sales_edit')){
			$this->show_access_denied_page();
		}
		$data=$this->data;
		$data['page_title']=$this->lang->line('sales_invoice');
		$data=array_merge($data,array('sales_id'=>$sales_id));
		$this->load->view('kitchen-invoice-pos',$data);
	}
