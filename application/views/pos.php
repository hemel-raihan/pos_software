      ...
      <div class="container">
        ...
        <div class="collapse navbar-collapse pull-left" id="navbar-collapse">
          <ul class="nav navbar-nav">
            ...
            <?php } ?>
            <?php if($CI->permissions('sales_view')) { ?>
            <li class=""><a href="<?php echo $base_url; ?>todaysales" title="View Todays Sales!"><i class="fa fa-list text-yellow" ></i> <span>Today's Sales</span></a></li>
            <?php } ?>
          ...
          <div class="box box-primary">
            ...
            <div class="box-header with-border" style="padding-bottom: 0px;">
              <div class="row" >
                ...
                <div class="col-md-8">
                <ul class="ul_class">
                        <li class="li_class"><input type="radio"  name="order_type" value="Dine In"  id="cb1" />
                            <label id="touch_dinein" class="label_class" for="cb1">Dine In</label>
                        </li>
                        <li class="li_class"><input type="radio"  name="order_type" value="Takeway" id="cb2" />
                            <label id="touch_takeway" class="label_class" for="cb2">Takeway</label>
                        </li>
                        <li  class="li_class"><input type="radio"   name="order_type" value="Delivery" id="cb3" />
                            <label id="touch_delivery" class="label_class" for="cb3">Delivery</label>
                        </li>
                </ul>
            ...
            <?php } ?>
              <div class="row">
                ...
                <div class="col-md-4">
                  <div class="input-group">
                    <span class="input-group-addon" title="Waiter"><i class="fa fa-user"></i></span>
                     <select class="form-control select2" id="waiter_id" name="waiter_id"  style="width: 100%;" onkeyup="shift_cursor(event,'expense_for')" >
                         <option value="">Select Waiter</option>
                        <?php
                        $query1="select * from db_users where role_id=2";
                        $q1=$this->db->query($query1);
              ...
              <div class="row">
                    <div class="col-xs-12 ">
                      <div class="col-md-4">
                          ...
                          <!--<label style="font-size: 12px">Delivery:</label><br>-->
                          <!--<?= $CI->currency('<span style="font-size: 15px;" class="tot_service text-bold"></span>');?>-->
                          <!-- </div>-->
                        </div>
                        <div class="col-md-8">
                          <div class="form-group">
                              ...
                              <!-- --------------------this code is for service charge Percentage-------------------- -->
                              
                               <div class="delivery_charge" style="display: none;">
                                <label for="other_charges" class="col-sm-4 control-label">Delivery Charge</label>

                                <!-- <div class="col-sm-4">
                                <div class="form-group">
                                  <select class="form-control" id='charges_type' name="charges_type">
                                    <option value='in_percentage'>Per%</option>
                                    <option value='in_fixed'>Fixed</option>
                                  </select>
                                </div>
                                ...
                                <input type="hidden" id="charges_type" name="charges_type" value="in_fixed">

                                <div class="col-sm-4">
                                <input type="text"  class="form-control text-right" id="other_charges" name="charges_amt" placeholder="0.00">
                                <input type="hidden" id="ot_charges"   name="other_charges" placeholder="0.00"  value="">
              ...
              <div class="box-footer bg-gray">
                <div class="row">
                  ...
                  <div class="col-md-2 text-right">
                          <label>Gov Sd:</label><br>
                          <?= $CI->currency('<span style="font-size: 19px;" class="gov_sd text-bold"></span>');?>
                          <input type="hidden" id="hidden_dt" name="gov_sd_data" value="">
                  </div>
                  <div class="col-md-2 text-right">
                  ...
                  <input type="hidden" name="test" class="test" value="">
                </div>
                ...
                <div class="row">
                  ...
                  <div class="col-md-12 text-right">
                   ...
                   <!--  
                    ...
                    <!--<div class="col-sm-2">-->
                    <!--  <button type="button" id="kitchen_slip" name="" class="btn bg-purple btn-block btn-flat btn-lg ctrl_a" title="By Cash & Save [Ctrl+A]">-->
                    <!--        <i class="fa fa-money" aria-hidden="true"></i>-->
                    <!--         KOT-->
                    <!--      </button>-->
                    <!--</div>-->
                    ...
                    <div class="col-sm-3">
                      <button type="button" id="guest_hold_invoice" name="" class="btn bg-purple btn-block btn-flat btn-lg ctrl_a" title="By Cash & Save [Ctrl+A]">
                            <i class="fa fa-money" aria-hidden="true"></i>
                             Guest Bill
                          </button>
                    </div>
...
$("#touch_delivery").click(function(){
    $("div.delivery_charge").show();
});
$("#touch_takeway").click(function(){
    $("div.delivery_charge").hide();
});
$("#touch_dinein").click(function(){
    $("div.delivery_charge").hide();
});
...
function addrow(id){
    ...
    var gov_amt   =$('#div_'+id).attr('data-item-gov-amt');
    ...
    var str=' <tr id="row_'+rowcount+'" data-row="0" data-item-id='+item_id+'>';/*item id*/
        ...
        if(gov_amt)
        {
          str+='<input type="hidden" name="tr_gov_amt_'+rowcount+'" id="tr_gov_amt_'+rowcount+'" value="'+gov_amt+'">';
        }
        else
        {
          str+='<input type="hidden" name="tr_gov_amt_'+rowcount+'" id="tr_gov_amt_'+rowcount+'" value="0">';
        }
        /*gov sd*/
        str+='<td id="td_'+rowcount+'_12"><input type="hidden" data-toggle="tooltip" title="Click to Change" id="td_data_'+rowcount+'_12" onclick="show_sales_item_modal('+rowcount+')" name="td_data_'+rowcount+'_12" type="text" class="form-control no-padding pointer min_width" readonly value="'+gov_amt+'"></td>';
...
function calulate_charges(other_charges,charges_type,total){
  if(charges_type=='in_percentage'){
    return parseFloat((total*other_charges)/100);
  }
  else{//in_fixed
    return parseFloat(other_charges);
  }
}
...
function final_total(){
  ...
  if($(".items_table tr").length>1){
    for(i=0;i<rowcount;i++){
      ...
      var sales_price = $("#sales_price_"+i).val();
      var sales_price = parseFloat(sales_price);
      gov_sd=$("#tr_gov_amt_"+i).val();
      var gov_sd = parseFloat(gov_sd);

      //gov_sdpercent = (sales_price*gov_sd)/parseFloat(100);
      var itemqty=$("#item_qty_"+item_id).val();
      var itemqty = parseFloat(itemqty);
      var test = gov_sd * itemqty;
      var test = parseFloat(test);
      total=parseFloat(total) + + + test + + +parseFloat($("#td_data_"+i+"_4").val()).toFixed(2);
      var unit_price = total - (tax_amt+test);
    ...
    //   gov_sd_total=$('#td_data_'+i+'_12').val();
    //   var gov_sd_total = gov_sd_total * itemqty;
    //   var gov_sd_total = parseFloat(gov_sd_total);
    
    //   gov_sd_total =parseFloat(gov_sd_total) + + + $('#td_data_'+i+'_12').val();
    //   var gov_sd_total = gov_sd_total * itemqty;
       gov_sd_tota=$('#td_data_'+i+'_12').val();
       var gov_sd_tota = gov_sd_tota * itemqty;
       var gov_sd_total =parseFloat(gov_sd_total) + + + parseFloat(gov_sd_tota);
      }
    }//for end
  }//items_table
  var other_charges=calulate_charges(other_charges,charges_type,total);
      other_charges = (isNaN(other_charges)) ? parseFloat(0) :other_charges;
  total+=other_charges;
  
  total =round_off(total);
  $("#ot_charges").val(other_charges);
  
  var discount_amt=0;
  if(total>0){
    var discount_amt=calulate_discount(discount_input,discount_type,unit_price);//return value 
  }


  set_total(item_qty,other_charges,gov_sd_total,total,discount_amt,total-discount_amt);
}
function set_total(tot_qty=0, other_charges=0, gov_sd_total=0, tot_amt=0, tot_disc=0, tot_grand=0){
  $(".tot_qty   ").html(tot_qty);
  $(".gov_sd   ").html((round_off(gov_sd_total).toFixed(2)));
  $("#hidden_dt").val((round_off(gov_sd_total).toFixed(2)));
  $(".tot_amt   ").html((round_off(tot_amt).toFixed(2)));
  $(".tot_service").html((round_off(other_charges).toFixed(2)));
...
function adjust_payments(){
  ...
  $("#change_amount_return").val((change_return).toFixed(2)); 
