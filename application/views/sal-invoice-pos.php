...
// 	$total_paying = $res->total_paying;
    ...
    $company_vat_number		=$res1->vat_no;//Goods and Service Tax Number (issued by govt.)
  	...
  	$q3=$this->db->query("SELECT a.customer_name,a.mobile,a.phone,a.gstin,a.tax_number,a.email,
                           ...
                           b.sales_code,b.sales_note, b.order_type, b.waiter_id,
	...
	<table width="98%" align="center">
		<tr>
			<td align="center">
				...
				<img class='img-responsive' width="200px"  src="<?php echo $base_url; ?>/uploads/company/<?= $company_logo;?>">
		...
		<?php if($order_type == 'Dine In')
			{
				echo "<h3><b>	Token No: $sales_id</b></h3>";
			}
			else
			{
				echo "<h3><b>	Type: $order_type</b></h3>";
			} 
			?>
		...
		<tr>
			<td>
				...
				<!--		<th style="font-size: 11px; text-align: right;padding-left: 2px; padding-right: 2px;"><?= $this->lang->line('discount'); ?></th>  -->
	                ...
	                <tr>
						<td style=" padding-left: 2px; padding-right: 2px;" colspan="4" align="right">SD (<?= $gov_sd_percant ?>%)</td>
					...
					$q=$this->db->query("select * from db_salespayments where sales_id=$sales_id");
					//$res=$q->row();
					foreach ($q->result() as $res22) {
						$payment_type = $res22->payment_type;
						$change_return = $res22->change_return;
						$total_paying = $res22->total_paying;
	...
	<table width="98%" align="center">
		...
		<tr>
			...
			<table width="100%">
					...
					<tr>
						<td >Order Time: <?= $sales_date." ".$created_time; ?></td>	
					</tr>
					<tr>
					<td >Print Time: <?= $print_date; ?></td>
					</tr>
				</table>
			</td>
