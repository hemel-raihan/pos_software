...
class Reports_model extends CI_Model {
	...
	public function show_item_sales_report(){
		...
		$this->db->select("a.id,a.sales_code,a.sales_date,a.subtotal,b.customer_name,b.customer_code,c.total_cost,c.price_per_unit");
		$this->db->select("c.sales_qty,d.item_name,c.tax_amt,c.gov_amt");
...
// 		}
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				echo "<td>".$res1->customer_name."</td>";
				echo "<td>".$res1->item_name."</td>";
				echo "<td>".$res1->price_per_unit."</td>";
				echo "<td class='text-right'>".$res1->tax_amt."</td>";
				echo "<td class='text-right'>".$res1->gov_amt."</td>";
				echo "<td class='text-right'>".$res1->sales_qty."</td>";
				echo "<td class='text-right'>".app_number_format($res1->total_cost+$res1->gov_amt)."</td>";
				
				echo "</tr>";
				$tot_total_vat+=$res1->tax_amt;
				$tot_total_sd+=$res1->gov_amt;
				$tot_total_cost+=$res1->total_cost+$res1->gov_amt;
				$tot_total_qty+=$res1->sales_qty;
				
			}

			echo "<tr>
					  <td class='text-right text-bold' colspan='7'><b>Vat :</b>"." ".app_number_format($tot_total_vat)."</td>
					  <td class='text-right text-bold'><b>SD :</b>"." ".app_number_format($tot_total_sd)."</td>
					  <td class='text-right text-bold'><b>Total Quantuty :</b>"." ".app_number_format($tot_total_qty)."</td>
					  <td class='text-right text-bold' ><b>Total :</b> "." ".app_number_format($tot_total_cost)."</td>
				  </tr>";
		}
		else{
			echo "<tr>";
...
//		$this->db->group_by("c.`item_id`");
	...
	public function show_sales_payments_report(){
		...
		$to_date=date("Y-m-d",strtotime($to_date));
		$payment_type = $this->input->post('payment_type');
		$this->db->select("c.id,c.sales_code,a.payment_date,b.customer_name,b.customer_code,a.payment_type,a.payment_note,a.payment,a.tax_amt,a.gov_amt");
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				echo "<td>".$res1->payment_type."</td>";
				echo "<td>".$res1->payment_note."</td>";
				echo "<td>".$res1->tax_amt."</td>";
				echo "<td>".$res1->gov_amt."</td>";
				echo "<td class='text-left'>".app_number_format($res1->payment)."</td>";
				echo "</tr>";
				$tot_tax+=$res1->tax_amt;
				$tot_sd+=$res1->gov_amt;
				$tot_payment+=$res1->payment;
			}
			...
			echo "<tr>
					   <td class='text-right text-bold' colspan='7'><b>Total :</b></td>
					  <td class='text-left text-bold'>".app_number_format($tot_tax)."</td>
					  <td class='text-left text-bold'>".app_number_format($tot_sd)."</td>
					  <td class='text-left text-bold'>".app_number_format($tot_payment)."</td>
				  </tr>";
		}
		else{
			echo "<tr>";
