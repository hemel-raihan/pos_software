...
class Pos_model extends CI_Model {
		...
		extract($_POST);
	      ...
	      if($q2->num_rows()>0){
	        foreach($q2->result() as $res2){
				...
				$item_gov_amt = $res2->gov_amt;
	        	$item_sales_price = $res2->price;
	          ...
	          <div class="box box-default item_box" id="div_'.$res2->id.'" onclick="'.$str.'"
							...
							data-item-gov-amt="'.$item_gov_amt.'"
	           	...
	           	<span class="label label-danger push-right" style="font-weight: bold;font-family: sans-serif;" data-toggle="tooltip" title="'.$res2->stock.' Quantity in Stock">Code: '.$res2->item_code.'</span>
	...
	public function pos_save_update(){//Save or update sales
		...
		$test =$this->xss_html_filter(trim($_REQUEST['test']));
		...
		for($i=0;$i<$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && trim($_REQUEST['tr_item_id_'.$i])!=''){
				...
				$gov_amt =$this->xss_html_filter(trim($_REQUEST['td_data_'.$i.'_12']));
		...
		for($i=0;$i<$rowcount;$i++){
			$sales_qty 	=$this->xss_html_filter(trim($_REQUEST['item_qty_'.$item_id]));
			$tax_amt =$this->xss_html_filter(trim($_REQUEST['td_data_'.$i.'_11']));
			$gov_amt =$this->xss_html_filter(trim($_REQUEST['gov_sd_data']));
		...
		for($i=1;$i<=$payment_row_count;$i++){
			...
			if((isset($_REQUEST['amount_'.$i]) && trim($_REQUEST['amount_'.$i])!='') || ($by_cash==true)){
				...
				if($by_cash==true){
					...
					$change_amount 	=$this->xss_html_filter(trim($_REQUEST['change_amount']));
				}
				...
				{
					$change_return =$tot_grand-$amount;
					$amount1 =$tot_grand;
				}
				//end
	    ...
	    if($q2->num_rows()>0){
		  ...
		  if($rows>0){
		  	...
		  	foreach ($q3->result() as $res3) { 
                ...
                echo  '<td id="td_'.$i.'_12"><input type="hidden" data-toggle="tooltip" title="Click to Change" id="td_data_'.$i.'_12" onclick="show_sales_item_modal('.$i.')" name="td_data_'.$i.'_12" type="text" class="form-control no-padding pointer min_width" readonly value="'.$gov_amt.'"></td>';

                if($gov_amt)
				{
					echo '<input type="hidden" name="tr_gov_amt_'.$i.'" id="tr_gov_amt_'.$i.'" value="'.$gov_amt.'">';
				}
				else
				{
					echo '<input type="hidden" name="tr_gov_amt_'.$i.'" id="tr_gov_amt_'.$i.'" value="0">';
				}
