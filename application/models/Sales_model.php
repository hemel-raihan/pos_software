...
class Sales_model extends CI_Model {
	...
	var $column_order = array( 'a.return_bit','a.id','a.sales_date','a.sales_code','a.reference_no','a.grand_total','a.payment_status','a.created_by','a.soft_del','b.customer_name','a.paid_amount','a.sales_status','a.pos'); //set column field database for datatable orderable
...
// 		{
// 			$this->db->select($this->column_order);
// 			$this->db->where('a.soft_del IS NULL');
// 		}
		$this->db->select($this->column_order);
	...
	function update_sales_payment_status_by_sales_id($sales_id,$customer_id){
		...
		// $q8=$this->db->query("select COALESCE(SUM(payment),0) as payment from db_salespayments where sales_id='$sales_id'");
			$q8=$this->db->query("select payment as payment from db_salespayments where sales_id='$sales_id'");
	...
	public function delete_sales($ids){
        ...
        //$q3=$this->db->query("update db_sales set soft_del=1 where id in($ids)");
